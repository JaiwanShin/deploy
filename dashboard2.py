...
def create_price_quintile_chart(df, calmf_unit_price, market_median):
    ...
    for _, row in summary.iterrows():
        bucket_prices = df_analysis[df_analysis["bucket"] == row["bucket"]]["unit_price"]
        if bucket_prices.min() <= calmf_unit_price <= bucket_prices.max():
...
def main():
    ...
    if not calmf_products.empty:
        ...
        if not analysis_df.empty:
            ...
            with col2:
                st.markdown("""
                <h5 style='color: #ffffff;'>Band ±20% (₩{:.0f} ~ ₩{:.0f})
                    <span class="kpi-help" data-tooltip="캄프 단가의 80%~120% 범위 내 경쟁자&#10;확장 경쟁 상품군">?</span>
                </h5>
                """.format(calmf_unit_price * 0.8, calmf_unit_price * 1.2), unsafe_allow_html=True)
                
                band20_df = analysis_df[
                    (analysis_df["unit_price"] >= calmf_unit_price * 0.8) &
                    (analysis_df["unit_price"] <= calmf_unit_price * 1.2)
                ]
                render_competition_table(band20_df, "Band 20%", highlight_top=3)
    ...
    if not calmf_products.empty and "analysis_df" in dir() and not analysis_df.empty:
        ...
        with col2:
            ...
            if quintile_summary is not None and not quintile_summary.empty:
                render_dark_table(
                    quintile_summary,
                    columns=["bucket", "n", "median_unit_price", "premium_index", "median_rank"],
                    rename_cols={
                        "bucket": "분위",
                        "n": "상품수",
