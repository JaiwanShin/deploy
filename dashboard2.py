...
def render_competition_table(df, title, highlight_top=3, sort_by="page_rank"):
    """경쟁 밴드 테이블 렌더링 (상위 N개 하이라이트, 캄프 강조)
    
    Args:
        sort_by: 정렬 기준 - 'page_rank' 또는 'unit_price'
    """
    if df.empty:
        st.info("해당 조건의 상품이 없습니다.")
        return
    
    df_sorted = df.sort_values(sort_by).head(20).copy()
...
def render_head_to_head_table(df, calmf_unit_price, calmf_rank, price_range=0.15, segment_filter="Premium"):
    """캄프 Head-to-Head 경쟁자 테이블 (단가 ±15% 내 동일 세그먼트 경쟁자)"""
    if df.empty:
        st.info("해당 조건의 상품이 없습니다.")
        return
    ...
    # ±15% 범위 + 세그먼트 필터링
    base_filter = (
        (df["unit_price"] >= calmf_unit_price * (1 - price_range)) &
        (df["unit_price"] <= calmf_unit_price * (1 + price_range)) &
        (df["unit_price"].notna()) &
        (df["page_rank"].notna())
    )
    ...
    if segment_filter and "segment" in df.columns:
        base_filter = base_filter & (df["segment"] == segment_filter)
    
    competitors = df[base_filter].copy()
    
    if competitors.empty:
        st.info(f"±{int(price_range*100)}% 가격 범위 내 {segment_filter} 세그먼트 경쟁자가 없습니다.")
        return
    
    # 단가차(%), 랭크차 계산
    competitors["price_diff_pct"] = ((competitors["unit_price"] - calmf_unit_price) / calmf_unit_price * 100)
    competitors["rank_diff"] = competitors["page_rank"] - calmf_rank
    
    # 랭크 순 정렬 (좋은 순)
    competitors_sorted = competitors.sort_values("page_rank").head(15)
    
    # HTML 테이블 생성
    html = f'<p style="color: #b2bec3; font-size: 0.85rem;">캄프 단가 ±{int(price_range*100)}% 범위 내 <strong style="color: #f39c12;">{len(competitors)}</strong>개 경쟁자</p>'
    html += '<table class="dark-table">'
    html += '<thead><tr>'
    cols = ["", "랭크", "브랜드", "상품명", "단가(₩)", "단가차", "랭크차"]
    for col in cols:
        html += f'<th>{col}</th>'
    html += '</tr></thead><tbody>'
    
    for _, row in competitors_sorted.iterrows():
        brand = str(row.get("brand", "")).lower()
        product_name = str(row.get("product_name", "")).lower()
        is_calmf = "캄프" in brand or "calmf" in brand or "캄프" in product_name or "calmf" in product_name
        ...
        if is_calmf:
            row_style = "background: rgba(241, 196, 15, 0.4);"
            star = "⭐"
        elif row["rank_diff"] < 0:  # 캄프보다 랭크 좋은 경쟁자
            row_style = "background: rgba(231, 76, 60, 0.2);"  # 빨간색 (위협)
            star = "⚠️"
        else:
            row_style = ""
            star = ""
        ...
        if rank_diff < 0:
            ...
            rank_diff_color = "#e74c3c"  # 위협적
        elif rank_diff > 0:
            rank_diff_str = f"+{rank_diff}위 ↓"
            rank_diff_color = "#27ae60"  # 안전
        else:
            rank_diff_str = "동일"
        ...
        html += f'<tr style="{row_style}">'
        html += f'<td style="text-align: center;">{star}</td>'
        html += f'<td>{int(row["page_rank"])}</td>'
        html += f'<td>{"<strong>" if is_calmf else ""}{row.get("brand", "N/A")}{"</strong>" if is_calmf else ""}</td>'
        html += f'<td style="max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{str(row.get("product_name", "N/A"))[:35]}</td>'
        html += f'<td style="text-align: right;">{row["unit_price"]:,.1f}</td>'
        html += f'<td style="text-align: right; color: {price_diff_color};">{price_diff_str}</td>'
        html += f'<td style="text-align: right; color: {rank_diff_color};">{rank_diff_str}</td>'
        html += '</tr>'
...
def create_pareto_frontier_chart(df, calmf_unit_price, calmf_rank):
    """Pareto Frontier 가치 맵 (가격 vs 랭크) + dominated_by 계산"""
    if df.empty or "unit_price" not in df.columns or "page_rank" not in df.columns:
        return None, 0
    
    df_plot = df.dropna(subset=["unit_price", "page_rank"]).copy()
    
    if df_plot.empty:
        return None, 0
    
    # dominated_by 계산: 캄프보다 싸거나 같고 + 랭크도 더 좋은 상품 수
    dominated_by = len(df_plot[
        (df_plot["unit_price"] <= calmf_unit_price) & 
        (df_plot["page_rank"] < calmf_rank)
    ])
    
    # Pareto frontier 계산 (더 싸면서 더 잘 보이는 상품들)
    df_sorted = df_plot.sort_values("unit_price")
    pareto_points = []
    min_rank = float('inf')
    ...
    fig.add_trace(go.Scatter(
        x=df_plot["unit_price"],
        y=df_plot["page_rank"],
        mode="markers",
        marker=dict(size=8, color="#3498db", opacity=0.5),
        name="시장 상품",
        hovertemplate="단가: ₩%{x:.0f}<br>랭크: %{y}<extra></extra>"
    ))
    
    # Pareto frontier 라인
    if not pareto_df.empty:
        fig.add_trace(go.Scatter(
            x=pareto_df["unit_price"],
            y=pareto_df["page_rank"],
            mode="lines+markers",
            line=dict(color="#e74c3c", width=2, dash="dash"),
            marker=dict(size=10, color="#e74c3c"),
            name="Pareto Frontier",
            hovertemplate="<b>효율 선두</b><br>단가: ₩%{x:.0f}<br>랭크: %{y}<extra></extra>"
        ))
    ...
    fig.add_trace(go.Scatter(
        ...
        name="캄프",
        hovertemplate="<b>캄프</b><br>단가: ₩%{x:.0f}<br>랭크: %{y}<extra></extra>"
    ))
    
    fig.update_layout(
        title="",
        xaxis_title="1매당 단가 (₩)",
        yaxis_title="Page Rank (낮을수록 좋음)",
        yaxis_autorange="reversed",  # 랭크 낮을수록 위로
        plot_bgcolor="rgba(0,0,0,0)",
        paper_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#ffffff"),
        height=400,
...
def main():
    ...
    with col_left:
        # 오른쪽 차트들 (세그먼트 분포 + 랭크별 분포) 사이에 수직 중앙 정렬을 위한 여백
    ...
    with col1:
        st.markdown("""
        <h5 style='color: #ffffff;'>캄프 가격대 경쟁 브랜드 SOV
            <span class="kpi-help" data-tooltip="📐 수식: 가중 SOV = Σ(1/√랭크)&#10;&#10;캄프 단가 ±15% 범위 내 브랜드별 노출 점유율&#10;제곱근 가중치로 상위 랭크 과대평가 방지">?</span>
        </h5>
        """, unsafe_allow_html=True)
        st.markdown("""
        <p class="metric-desc">
            캄프와 <strong>유사 가격대(±15%)</strong>에서 어떤 브랜드가 가장 강한지 보여줍니다.
            상위 브랜드의 상품 구성/마케팅을 벤치마킹하세요.
        </p>
        """, unsafe_allow_html=True)
        
        # 캄프 가격대 필터링 후 브랜드별 SOV 계산
        calmf_products = data["calmf_products"]
        if not calmf_products.empty and not df_main.empty:
            calmf_unit_price = calmf_products.iloc[0]["unit_price"]
            
            # 캄프 가격대 ±15% 필터링
            price_band_df = df_main[
                (df_main["unit_price"] >= calmf_unit_price * 0.85) &
                (df_main["unit_price"] <= calmf_unit_price * 1.15) &
                (df_main["unit_price"].notna()) &
                (df_main["page_rank"].notna())
            ].copy()
            
            if not price_band_df.empty and "brand" in price_band_df.columns:
                # 브랜드별 가중 SOV 계산 (1/√랭크 - 제곱근 가중치)
                price_band_df["weight"] = 1 / np.sqrt(price_band_df["page_rank"])
                brand_sov = price_band_df.groupby("brand").agg(
                    weighted_sov=("weight", "sum"),
                    item_count=("brand", "count")
                ).reset_index()
                brand_sov = brand_sov.sort_values("weighted_sov", ascending=False)  # 전체 브랜드 표시
                
                # 캄프 하이라이트
                brand_sov["is_calmf"] = brand_sov["brand"].str.lower().str.contains("캄프|calmf", na=False)
                colors = ["#f1c40f" if c else "#3498db" for c in brand_sov["is_calmf"]]
                
                # 차트 높이 동적 조정 (브랜드 수 × 30px, 최소 250px)
                chart_height = max(250, len(brand_sov) * 30)
                ...
                fig_sov = px.bar(
                    ...
                    orientation="h",
                    title="",
                    labels={"weighted_sov": "가중 SOV (1/√랭크 합)", "brand": "브랜드"},
                    text=brand_sov["weighted_sov"].apply(lambda x: f"{x:.2f}")
                )
                fig_sov.update_traces(marker_color=colors, textposition="outside")
                ...
                # 캄프 순위 및 SOV 설명
                calmf_sov_row = brand_sov[brand_sov["is_calmf"]]
                total_brands = len(brand_sov)
                if not calmf_sov_row.empty:
                    calmf_rank = brand_sov[brand_sov["is_calmf"]].index[0] + 1
                    calmf_sov_value = calmf_sov_row["weighted_sov"].values[0]
                    st.markdown(f"""
                    <div class="insight-box">
                        <strong>📍 캄프:</strong> SOV <strong>{calmf_rank}위</strong> / {total_brands}개 브랜드 (가중치 {calmf_sov_value:.2f})<br>
                        <span style="font-size: 0.85rem; color: #b2bec3;">
                        💡 SOV 숫자 의미: 1위 상품 1개 = 1.0, 4위 상품 1개 = 0.5, 79위 = 0.11
                        </span>
                    </div>
                    """, unsafe_allow_html=True)
            else:
                st.info("브랜드 SOV 계산을 위한 데이터가 없습니다.")
        else:
    ...
    # ==========================================================================
    # Head-to-Head 경쟁자 분석 섹션 (신규)
    # ==========================================================================
    
    render_section_header(
        "Head-to-Head Competitors",
        "캄프와 가격대가 유사한(±15%) 직접 경쟁자들의 랭크 비교",
        "🎯"
    )
    ...
    if not calmf_products.empty and not df_main.empty:
        ...
        with col1:
            st.markdown("<h5 style='color: #ffffff;'>Premium 세그먼트 직접 경쟁자 (단가 ±15%)</h5>", unsafe_allow_html=True)
            st.markdown("""
            <p class='metric-desc'>
                ⚠️ = 캄프보다 랭크 좋은 위협적 경쟁자 | ⭐ = 캄프 | Premium 세그먼트만
            </p>
            """, unsafe_allow_html=True)
            render_head_to_head_table(df_main, calmf_unit_price, calmf_rank, price_range=0.15, segment_filter="Premium")
        
        with col2:
            # 핵심 인사이트 (Premium만)
            if "unit_price" in df_main.columns and "page_rank" in df_main.columns and "segment" in df_main.columns:
                competitors_15 = df_main[
                    (df_main["unit_price"] >= calmf_unit_price * 0.85) &
                    (df_main["unit_price"] <= calmf_unit_price * 1.15) &
                ...
                better_rank = len(competitors_15[competitors_15["page_rank"] < calmf_rank])
                same_or_cheaper = len(competitors_15[competitors_15["unit_price"] <= calmf_unit_price])
                
                # 테이블 시작과 맞추기 위한 여백
                st.markdown("<div style='margin-top: 90px;'></div>", unsafe_allow_html=True)
                st.markdown("<h5 style='color: #ffffff;'>📊 Premium 경쟁 요약</h5>", unsafe_allow_html=True)
                
                render_kpi(better_rank, "랭크 우위 경쟁자", f"Premium {len(competitors_15)}개 중")
                render_kpi(same_or_cheaper, "같거나 저렴한 상품", "캄프 단가 이하 Premium 경쟁자")
                
                # 인사이트 박스
                if better_rank > len(competitors_15) * 0.5:
                    st.markdown(f"""
                    <div class="insight-box warning">
                        <strong>⚠️ 분석:</strong> 비슷한 가격대 {len(competitors_15)}개 중 
                        <strong>{better_rank}개</strong>({better_rank/len(competitors_15)*100:.0f}%)가 
                        캄프보다 상위 노출<br><br>
    ...
    # ==========================================================================
    # Pareto Frontier 가치 맵 섹션 (신규)
    # ==========================================================================
    
    render_section_header(
        "Price × Rank Value Map",
        "가격 대비 노출 효율성 분석 (Pareto Frontier)",
        "📈"
    )
    ...
    if not calmf_products.empty and not df_main.empty:
        ...
        with col1:
            fig_pareto, dominated_by = create_pareto_frontier_chart(
                df_main, calmf_unit_price, calmf_rank
            )
            if fig_pareto:
                st.plotly_chart(fig_pareto, use_container_width=True)
            else:
                st.info("Pareto 분석을 위한 데이터가 부족합니다.")
        
        with col2:
            st.markdown("<h5 style='color: #ffffff;'>효율성 지표</h5>", unsafe_allow_html=True)
            
            total_products = len(df_main[df_main["unit_price"].notna() & df_main["page_rank"].notna()])
            
            # dominated_by KPI (핵심!)
            render_kpi(
                dominated_by, 
                "Dominated By", 
                "캄프보다 싸거나 같고 + 랭크도 좋은 상품",
                tooltip="숫자가 클수록 가격 대비 노출 경쟁력이 낮음"
            )
            
            # 퍼센트로도 표시
            if total_products > 0:
                dominated_pct = dominated_by / total_products * 100
                st.markdown(f"""
                <div class="kpi-card">
                    <div class="kpi-value" style="font-size: 1.8rem; background: linear-gradient(90deg, 
                        {'#e74c3c, #c0392b' if dominated_pct > 30 else '#27ae60, #2ecc71'}); -webkit-background-clip: text;">
                        {dominated_pct:.1f}%
                    </div>
                    <div class="kpi-desc">전체 {total_products}개 중</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            if dominated_by > 0:
                ...
                <div class="insight-box warning" style="margin-top: 15px;">
                    <strong>해석:</strong> 캄프는 시장 {total_products}개 상품 중 
                    <strong>{dominated_by}개</strong>에 의해 "지배"됨 
                    (더 싸거나 같으면서 랭크도 좋음).<br><br>
                    <strong>📌 액션 포인트:</strong><br>
                    {'• 가격 경쟁력 확보가 어려운 상황 → <strong>차별화 전략</strong> 필요<br>• Pareto 선상 경쟁자의 성공 요인 분석 권장' if dominated_pct > 30 else '• 현재 가격 대비 노출 수준은 보통<br>• 소폭 가격 조정 또는 노출 최적화로 개선 가능'}
                </div>
                """, unsafe_allow_html=True)
    else:
        st.info("Pareto 분석을 위한 데이터가 없습니다.")
    ...
    if not calmf_products.empty:
        ...
        if not analysis_df.empty:
            ...
            with col1:
                ...
                render_competition_table(band10_df, "Band 10%", highlight_top=3, sort_by="unit_price")
            ...
            with col2:
                ...
                # 캄프 제품을 추가해서 비교 (하이라이트용)
                calmf_in_data = analysis_df[
                    (analysis_df["brand"].str.lower().str.contains("캄프|calmf", na=False)) |
                    (analysis_df["product_name"].str.lower().str.contains("캄프|calmf", na=False))
                ]
                if not calmf_in_data.empty:
                    band20_with_calmf = pd.concat([band20_df, calmf_in_data]).drop_duplicates()
                else:
                    band20_with_calmf = band20_df
                
                render_competition_table(band20_with_calmf, "Band 10~20%", highlight_top=3, sort_by="unit_price")
    ...
    # ==========================================================================
    # Elasticity Analysis 섹션 (참고용 - 표본 부족으로 신뢰도 낮음)
    # ==========================================================================
    
    render_section_header(
        "Price-Rank Elasticity (참고용)",
        "⚠️ 표본 수 부족으로 통계적 검정력이 낮습니다. 방향성 참고용으로만 활용하세요.",
        "📐"
    )
