...
with tab2:
    ...
    if category_mode == "main":
        ...
        # 세부 카테고리 선택
        sub_cat_options = [k for k in SHOPPING_SUBCATEGORIES.keys()]
        main_category = st.selectbox(
            "대분류 선택",
            options=sub_cat_options,
            index=sub_cat_options.index("화장품/미용") if "화장품/미용" in sub_cat_options else 0
        )
...
with tab3:
    ...
    if st.session_state.analysis_results.get("tab3"):
        ...
        col1.metric("최저가", f"{df_valid['lprice'].min():,.2f}원")
        col2.metric("최고가", f"{df_valid['lprice'].max():,.2f}원")
        col3.metric("평균가", f"{df_valid['lprice'].mean():,.2f}원")
        col4.metric("중앙값", f"{df_valid['lprice'].median():,.2f}원")
        ...
        fig.add_vline(x=df_valid["lprice"].median(), line_dash="dash", line_color="#4ade80", annotation_text=f"중앙값: {df_valid['lprice'].median():,.2f}원")
        fig.update_layout(
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            font_family="Pretendard",
        ...
        with st.expander("📋 상품 목록 보기"):
            ...
            display_df["가격"] = display_df["가격"].apply(lambda x: f"{x:,.2f}원")
...
with tab4:
    ...
    # ===== 타겟 고객층 분석 =====
        ...
        if st.session_state.analysis_results.get("tab4_demo"):
            ...
            if not gender_df.empty:
                ...
                st.plotly_chart(fig, use_container_width=True)
                st.info("ℹ️ **지수(Index)**: 검색 빈도에 따른 상대적 비율입니다.")
    ...
    # ===== 상품 가격 비교 =====
        ...
        if st.session_state.analysis_results.get("tab4_price"):
            ...
            st.dataframe(s_df.style.format({"최저가": "{:,.2f}원", "최고가": "{:,.2f}원", "평균가": "{:,.2f}원", "중앙값": "{:,.2f}원"}), use_container_width=True)
...
with tab6:
    ...
    if keyword_mode == "related":
        seed_keyword = st.text_input("시드 키워드 입력", value="카밍패드", key="seed_kw_input")
    ...
    # ===== 카테고리별 인기 키워드 =====
        ...
        with col_cat1:
            cat_options = list(SHOPPING_CATEGORIES.keys())
        ...
        with col_cat2:
            # 세부 카테고리가 있으면 표시
            if selected_category in SHOPPING_SUBCATEGORIES:
                subcats = list(SHOPPING_SUBCATEGORIES[selected_category].keys())
        ...
        default_keywords_map = {
            ...
            "화장품/미용": "캄프 카밍패드, 스킨케어, 마스크팩, 클렌징, 선크림",
            "식품": "과일, 채소, 라면, 커피, 과자",
        ...
        # 세부 카테고리에 따른 기본 키워드
        if selected_subcat != "전체" and selected_subcat:
            default_kw = f"{selected_subcat}, {selected_subcat} 추천, {selected_subcat} 인기, {selected_subcat} 순위, {selected_subcat} 가격"
        else:
        ...
        if st.session_state.analysis_results.get("tab6_cat"):
            ...
            st.plotly_chart(fig, use_container_width=True)
            st.info("ℹ️ **트렌드 지수**: 카테고리 내 클릭 빈도의 상대적 지표(0~100)입니다.")
    ...
    # ===== 마케팅 키워드 추천 =====
        ...
        with col_input1:
            target_product = st.text_input(
                "핵심 키워드 (브랜드명 제외)",
                value="카밍패드",
                help="예: 토너패드, 수분크림, 마스크팩 (브랜드명은 제외하고 입력)"
            )
        with col_input2:
            exclude_brand = st.text_input(
                "제외할 브랜드명 (선택)",
                value="",
                help="결과에서 제외하고 싶은 브랜드명이 있다면 입력하세요"
            )
        
        auto_filter = st.checkbox("🚫 브랜드 키워드 자동 필터링 (Beta)", value=True, help="상품 검색 결과를 분석하여 감지된 브랜드명을 포함한 키워드를 자동으로 제외합니다.")
            ...
            with st.spinner("마케팅 키워드 분석 중..."):
                try:
                    ...
                    product_df = client.search_all_products(
                        query=f"{exclude_brand} {target_product}" if exclude_brand else target_product,
                        max_results=300,
                        sort="sim"
                    )
                    ...
                    if not keyword_df.empty:
                        # 1. 사용자 지정 브랜드 필터링
                        if exclude_brand:
                            original_len = len(keyword_df)
                            keyword_df = keyword_df[~keyword_df["keyword"].str.contains(exclude_brand, case=False)]
                            st.toast(f"사용자 지정 제외: {original_len - len(keyword_df)}개 필터링됨")

                        # 2. 자동 브랜드 필터링 (Beta)
                        if auto_filter and not product_df.empty:
                            # 상품 데이터에서 브랜드/제조사 추출
                            detected_brands = set()
                            if "brand" in product_df.columns:
                                detected_brands.update(product_df["brand"].dropna().unique())
                            if "maker" in product_df.columns:
                                detected_brands.update(product_df["maker"].dropna().unique())
                            
                            # 빈 문자열 삭제
                            detected_brands = {b for b in detected_brands if b and len(str(b)) > 1}
                            
                            if detected_brands:
                                before_len = len(keyword_df)
                                # 브랜드명이 포함된 키워드 제외
                                # 속도 최적화를 위해 정규식 패턴 생성
                                import re
                                try:
                                    # 특수문자 이스케이프 처리
                                    escaped_brands = [re.escape(str(b)) for b in detected_brands]
                                    pattern = '|'.join(escaped_brands)
                                    
                                    keyword_df = keyword_df[~keyword_df["keyword"].str.contains(pattern, case=False, na=False)]
                                    filtered_count = before_len - len(keyword_df)
                                    
                                    if filtered_count > 0:
                                        st.toast(f"브랜드 자동 필터링: {filtered_count}개 제외됨")
                                        with st.expander("🚫 자동 제외된 브랜드 목록 (확인용)"):
                                            st.write(list(detected_brands))
                                except Exception as e:
                                    st.error(f"필터링 오류: {e}")

                        # 3. 기초 필터링 (글자수)

                        # 너무 긴 키워드나 태그성 키워드 제외 (선택사항)
                        keyword_df = keyword_df[keyword_df["keyword"].str.len() > 1] # 1글자 제외
                        ...
                        with col1:
                            ...
                            if not low_comp.empty:
                                for _, row in low_comp.iterrows():
                                    st.code(f"{row['keyword']} ({row['monthly_total']:,.2f}회)")
                            else:
                        ...
                        if not mobile_keywords.empty:
                            for _, row in mobile_keywords.iterrows():
                                st.markdown(f"- **{row['keyword']}** - {row['monthly_total']:,.2f}회/월 (모바일 {row['mobile_ratio']:.2f}%)")
                        else:
                        ...
                        if not product_df.empty:
                            ...
                            if not df_valid.empty:
                                ...
                                col1, col2, col3 = st.columns(3)
                                col1.metric("저가 진입점", f"~{q1:,.2f}원")
                                col2.metric("중가 구간", f"{q1:,.2f}~{q3:,.2f}원")
                                col3.metric("프리미엄 구간", f"{q3:,.2f}원~")
...
with tab7:
    ...
    target_market = st.text_input("분석 시장", value="기초화장품", key="market_entry_input")
    ...
    if market_mode == "size":
        ...
        if st.session_state.analysis_results.get("tab7_size"):
            ...
            col2.metric("상품 수", f"{total_products:,.2f}")
            ...
            if not t_df.empty:
                ...
                st.plotly_chart(fig, use_container_width=True)
                st.info("ℹ️ **검색 트렌드**: 지난 1년간의 관심도 변화 추이(상대 값)입니다.")
        ...
        if st.session_state.analysis_results.get("tab7_comp"):
            ...
            col3.metric("평균 가격", f"{df_v['lprice'].mean():,.2f}원")
...
with tab8:
    ...
    if api_available:
        search_keywords = st.text_input("조회 키워드 (쉼표 구분, 최대 5개)", value="캄프 카밍패드, 마스크팩, 스킨케어", key="s_kw_input")
